...
const UploadBtn = ({ onFile, label='IMAGE', accept='image/*' }) => {
  ...
  return (
    <>
      ...
      <input ref={r} type="file" accept={accept} hidden onChange={e => { const f=e.target.files[0]; if(f) onFile(URL.createObjectURL(f)); e.target.value=''; }} />
    </>
  );
};
...
const GalCard = ({ g, editMode, onChange, onOpen, style }) => {
  ...
  return (
    ...
    >
      ...
      {editMode && (
        <>
          <button className="pf-upload" style={{ pointerEvents:'all' }} onClick={e => { e.stopPropagation(); fileRef.current?.click(); }}>
            <span style={{ fontFamily:F.m, fontSize:'.6rem', color:C.red }}>📁 CHANGER<br/><span style={{ opacity:.5 }}>IMAGE</span></span>
          </button>
          <input ref={fileRef} type="file" accept="image/*" hidden onChange={e => { const f=e.target.files[0]; if(f) onChange('src',URL.createObjectURL(f)); e.target.value=''; }} />
        </>
      )}
    </div>
  );
...
// Recursively convert all blob: strings in an object to base64
